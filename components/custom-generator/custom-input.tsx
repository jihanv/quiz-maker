...
import {
  ParagraphSuccessResponse,
  TParagraphSchema,
  paragraphSchema,
} from "@/lib/types";
import {
  downloadDocxFromItem,
  MultipleChoiceData,
} from "@/features/cloze-generator/fileDownloader";
...
export default function CustomInput() {
  ...
  const handleNext = () => {
    ...
    const paragraphs = rawParagraphs.map((para, paragraphIndex) => {
      const tokens =
        para.match(/[\p{L}\p{N}]+(?:[-’'][\p{L}\p{N}]+)*|[^\s\p{L}\p{N}]/gu) ??
        [];
                    ...
                    {paragraphs.map((p) => (
                      <p key={p.paragraphIndex} className="mb-4">
                        {p.words.map((w) => (
                          <span
                            key={w.position}
                            className={`cursor-pointer text-lg font-medium px-0 rounded ${
                              w.selected ? "bg-yellow-200" : ""
                            }`}
