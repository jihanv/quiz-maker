...
describe("replaceSingleLetterPeriodCombos", () => {
  it("replaces a single initial before a name when it is NOT part of a multi-initial sequence", () => {
  ...
  it("replaces a single letter at sentence end (intentionally)", () => {
    const input =
  ...
  it("does not replace inside acronym sequences like U.S.", () => {
    const input = "He moved to the U.S. in 2010.";
    const out = replaceSingleLetterPeriodCombos(input);
    expect(out).toBe(input);
  });

  it("does not replace inside acronym sequences like U.S.A.", () => {
    const input = "The U.S.A. is large.";
    const out = replaceSingleLetterPeriodCombos(input);
    expect(out).toBe(input);
  });

  it("does not replace initials sequences like J. K. Rowling (single-letter tokens inside a sequence)", () => {
    const input = "J. K. Rowling wrote books.";
    const out = replaceSingleLetterPeriodCombos(input);
    expect(out).toBe(input);
  });

  it("does not replace initials sequences with multiple spaces (A.  B.   C.)", () => {
    const input = "A.  B.   C. are letters.";
    const out = replaceSingleLetterPeriodCombos(input);
    expect(out).toBe(input);
  });

  it("does replace a standalone single initial elsewhere in the same string", () => {
    const input = "A. B. C. are letters. Person D. left.";
    const out = replaceSingleLetterPeriodCombos(input);
    expect(out).toBe("A. B. C. are letters. Person <<SINGLE_INITIAL:D>> left.");
  });

  it("does not replace lowercase single-letter dot", () => {
    const input = "Use x. as a variable, not X.";
    const out = replaceSingleLetterPeriodCombos(input);
    expect(out).toBe("Use x. as a variable, not <<SINGLE_INITIAL:X>>");
  });

  it("does not replace when dot is between digits (extra safety)", () => {
    const input = "Pi is 3.14 and e is 2.71.";
    const out = replaceSingleLetterPeriodCombos(input);
    expect(out).toBe(input);
  });

  it("handles punctuation after the dot (quotes) for standalone initials", () => {
